...
| Skill | Package README | Installable folder | Purpose | Implicit invocation |
|---|---|---|---|---|
...
Each package README explains the specific skill's purpose, install steps, usage examples, and verification commands. The installable skill folders should stay focused on runtime resources: `SKILL.md`, `agents/openai.yaml`, `scripts/`, `references/`, and `assets/` when needed.
...
The validation script reads [skills.json](skills.json), runs `quick_validate.py` for each skill, then runs package-specific checks such as pytest and ruff for `capture-idea`.
