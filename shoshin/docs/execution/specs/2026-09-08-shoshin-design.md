...
- 委派适配：默认主 agent 完成，任务复杂或可并行本身不构成委派理由。仅在执行环境允许、能说明独立审查或独立调查的具体收益，且收益值得额外 token 成本时，使用 Codex 原生子代理机制。优先最少必要代理与最小充分上下文，要求简洁证据输出，避免全量历史复制、重复调查和固定多轮审查；不新增预算调度框架，也不强迫用户逐次批准。无明显收益时直接执行。此约束落实用户 R22，不固定代理数量；只有实际隔离的环境才可称隔离，不能把本地共享目录描述为独立云 VM。
