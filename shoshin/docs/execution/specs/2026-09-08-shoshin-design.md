...
- **Verify the documented helper interface.** Under R28, new or modified helpers keep their documented command, parameters, working directory and runtime requirements current. Execute that exact invocation and rerun affected paths. Verify executable permissions for direct execution or the specified interpreter for interpreter invocation; an undocumented workaround does not qualify. [SOURCE: docs/prd-v0.1.md#additional-confirmation]
- **Preserve overall coverage status.** If required verification or safe verification of a correction is unfinished, report blocked even if some local corrections passed. List those corrections separately. Only complete agreed source/live coverage permits clean (no correction needed) or changed (local corrections verified). Explicit scope exclusions and non-applicable paths stay visible; unreachable required paths do not become passes.
- **Repair prerequisite omissions.** When source and execution identify a necessary prerequisite omitted from the map, correct the relevant recipe. Distinguish stable requirements such as entitlement or platform from a temporary outage recorded for this run. Preserve uncertainty when the distinction is not established.
