...
- [x] Complete neutral checkout repair execution: initially blocked by two automatic approval denials, with unchanged files recorded in the original snapshot. The user then explicitly authorized the temporary source/test edit; normal execution and independent checks completed in the authorized follow-up below. The original denials remain preserved as historical evidence.

At this checkpoint, the Feature and skill-creator entrypoint paths remained unexercised; the later Feature and authoring section records their bounded execution. Unavailable-owner/backend cases, deliberately shortened catalogs, fresh user tasks and repeated cross-model/host observations remain open. No invocation reliability percentage or full package acceptance follows from this batch. Shoshin's installed instructions are unchanged; the continuation corrects test expectations and records evidence.
...
### 2026-09-10 Authorized checkout follow-up

The user explicitly allowed modification of the identified temporary checkout source and tests. Continue the same native agent, preserving its prior investigation; no new context or implicit-selection sample is counted. The batch now has five contexts and nine turns. The new record, tests/evidence/shoshin-native-routing-followup.json, binds the unchanged original evidence snapshot by SHA-256 and records authorization, native tool activity, final fixture contents and independent verification. [SOURCE: docs/execution/specs/2026-09-08-shoshin-design.md#entrypoint-selection-and-routing-contract]

Normal apply_patch succeeded. Before modifying the source, the agent added four test methods and ran the resulting five-method suite: five expected assertion failures, exit code 1. It then moved notification sending after the accepted storage write while retaining the three-attempt limit and already-paid no-op. The same suite passed with exit code 0. The parent inspected the actual tool results, independently reran the final test source against the preserved original and final implementations in memory, and reproduced the five failures followed by five passing tests. Additional checks covered zero through four transient failures, notification timing and repeated calls. Only checkout.py and test_checkout.py changed; README and all visual fixture files remained identical, with no unexpected files.

The previous execution blocker is resolved. This verifies the local in-memory fixture contract and continuation after authorization; it does not extend the implicit-selection sample count, prove real payment integration or complete the remaining package-wide coverage listed above. No skill instructions or installation files changed.

Follow-up validation: aggregate checks passed again, including 46 document-governance tests, 18 Shoshin helper tests, Ruff and skill structure checks. Strict document validation passed with zero warnings. The new evidence's original-snapshot hash, final-file hashes, completed turn, unchanged scope and before/after outcomes were verified; whitespace checks passed.

### 2026-09-10 Feature and authoring routing

The user approved the recommended next step: verify Feature implementation and skill authoring with bounded temporary projects, including an unnamed authoring contrast. Three new native agents received neutral fixture paths, no parent conversation history and no reviewer expectations. Their actual catalog contained 52 skills; each inherited gpt-6-astra with xhigh effort. Source, tests and documentation inside the fixtures were authorized; no personal installation, remote operation or extra agent runtime was part of the request. [SOURCE: docs/execution/specs/2026-09-08-shoshin-design.md#entrypoint-selection-and-routing-contract]

- [x] Observe the unnamed implementation request read shoshin and feature.md before editing. The pre-edit announcement identifies jobs.state reuse, the existing transaction joining accepted rows with progress, control interfaces and verification scope. It is brief; do not infer that a full transition table or separately labeled ownership statement was announced before editing.
- [x] Inspect pause/resume implementation in the existing single-caller SQLite library. The original three tests pass; the final eight tests pass in native execution and parent rerun. An inspected test uses four independent Python processes to check pause, reopen without resume, explicit resume without importing, and reopen to complete the exact row sequence. Controls preserve progress and completed state, with no schema change or background worker.
- [x] Observe the explicit shoshin authoring request read shoshin and the current system skill-creator. These reads occurred in the same tool batch; this establishes compatible method use, not sequential discovery causality. The unnamed authoring request reads skill-creator directly without adding shoshin, which is sufficient for its scope.
- [x] Review both generated skills and their six-row log summaries. Both retain skipped and blocked separately, preserve an unknown skip reason, classify the contradictory lint result from exit code 1, and use not run for absent execution/status. Native and parent quick validators pass; both local references resolve. UI/policy metadata and input logs remain byte-identical; no quoted command is executed.
- [x] Preserve the actual requests, artifacts, event order, catalog, resource hashes, native results and parent checks in tests/evidence/shoshin-feature-authoring-routing.json. Only the seven intended fixture files changed, with no new or missing files. The explicit author performed a shared-memory search; retain its action/hash but exclude historical content and do not use it as evidence for the conclusions.
- [x] Add route-31 as the canonical sufficient-leaf authoring contrast with separate reviewer expectations. Its native fixture variant ran; the new canonical metadata-only request was not rerun as part of the prior 30-case exercise.

These are three bounded observations on one host/model, not a reliability percentage or completion of full package acceptance. The authoring executors received category requirements and sample logs, so their successful output does not test the generated skills on unseen requests. Feature verification establishes baseline and passing results for a new capability, not failing-before TDD evidence. No confirmed instruction defect warrants a shoshin source or installation change from these observations. The remaining work is unavailable-dependency/backend behavior, shortened catalogs, fresh user-task acceptance and broader repeated observations.

Validation: aggregate checks passed, including 46 document-governance tests, 18 Shoshin helper tests, Ruff and skill structure checks. Strict document validation passed with zero warnings. Integrity checks verified 31 paired canonical requests/reviewer records, three completed contexts, the actual 52-entry catalog, initial/final fixture hashes and the seven-file write scope. Parent quick validation of both generated skills and both local links passed; whitespace checks passed.

### 2026-09-10 Capability failures, shortened catalogs and a fresh desktop task

The user requested completion of the remaining entrypoint checks quoted from the previous report. Run five independent native contexts without parent history for bounded failure handling and supplied-catalog selection, plus one new ordinary desktop task through the supported app API. Preserve all attempts and distinguish the executed evidence layers. This section updates the remaining-work statement above; it does not reopen the canceled real-project pilot or claim full package acceptance. [SOURCE: docs/execution/specs/2026-09-08-shoshin-design.md#entrypoint-selection-and-routing-contract]

- [x] Exercise authoring with only shoshin in the permitted project skill inventory. The executor identifies unavailable skill-creator, completes two authorized local guidance edits and six log classifications, and verifies local structure and references. It preserves UI/policy metadata and logs, does not execute logged commands, and explicitly leaves the dedicated validator unrun. The host still exposes skill-creator outside this constrained scope; this is scoped dependency handling, not physical host absence.
- [x] Exercise visual comparison with the visual method available and the designated project browser endpoint refusing connections. Native Doctor and independent parent checks return exit 1 with connection refused. The executor reads visual-parity.md without running the generator, reports likely button movement as static inference, and withholds screenshot or parity claims. Fixture files remain unchanged. Failure handling is verified; rendered equivalence is not.
- [x] Execute three independent supplied-catalog variants: full descriptions, first 120 characters, and first 60 characters, applied uniformly to the same 18 entries and 14 requests. All 15 required-entrypoint decisions and 21 bypass/status/contextless decisions match their expectations; the six optional decisions use permitted alternatives. Retain route-18's direct-architect versus entrypoint difference and route-04's additional architect choice. Selection results are not downstream execution evidence.
- [x] Inspect effective native metadata, revealing that full descriptions remain injected in all three selection contexts. Therefore the 42 decisions are an offline interpretation exercise with a material contamination limit. Genuine host-generated truncation was not observed or induced, and causal robustness under hidden/full-description removal remains unverified. Add this evidence requirement to the reviewer rubric rather than treating supplied-only instructions as context isolation.
- [x] Create the ordinary projectless desktop task `01a08a42-fe3a-71c0-a841-7ee52201646b` with an unnamed engineering request and neutral temporary project. Native metadata records Codex Desktop 0.153.4, source=vscode, thread_source=agent_created_thread, gpt-6-astra/xhigh, and 52 exposed skills. No prior parent conversation is inherited. This is an API-created ordinary task, not a native subagent or manually typed UI trial.
- [x] Observe installed shoshin and Feature reads before any edits, followed by tdd and the evidence method. The pre-edit announcement describes jobs.state, persistence, no progress movement while paused, completed-state protection and verification. Original three tests pass; the expanded nine-test suite reports eight missing-method errors before implementation; all nine tests pass after implementation and in the parent rerun. Four independent parent Python processes additionally verify persisted pause, no implicit resume, explicit resume without imports, and exact once-only completion.
- [x] Send a status-only follow-up to the same ordinary task. It returns a brief result summary without tool calls or further fixture changes. The task's initial shared-memory query is observable; redact unrelated hit content while retaining its action and hashes. Do not call this a memory-free experiment.
- [x] Retain exact dispatches, requests, initial/final files, catalog variants, native events, settings, resource hashes, failure evidence and parent checks in tests/evidence/shoshin-entrypoint-boundaries.json. Only five intended temporary files changed; no fixture files were added or removed. The ten installed shoshin files remain byte-identical to source.

The requested categories now have recorded exercises at the stated layers. Actual host-generated description truncation remains an explicit evidence gap because these native contexts include full metadata; it is not a failed source behavior and is not marked passed. No confirmed shoshin instruction defect warrants a skill-body or installation change. Preserve the existing PRD, Spec and ADR decisions; this change adds evidence and tightens verification interpretation only.

Parent verification initially encountered a macOS /var versus /private/var containment assertion in the temporary collector. Resolving the fixture root corrected the checker; the rerun passed. This was a parent verification-script error, not a product failure. The collector also required handling a desktop delegation record without a tool call ID; correcting that parser did not rerun or replace the native experiments.

Validation: aggregate checks passed, including 46 document-governance tests, 18 Shoshin helper tests, Ruff and skill validation. Strict document validation passed with zero warnings. Evidence integrity checks passed for six contexts, seven turns, five exact native dispatches, 42 selection records, 39 initial files, five changed files, 52 exposed host skills, exact catalog prefix transformations and the tool-free status follow-up. Whitespace checks passed.

### 2026-09-10 Verification maintenance restoration

The user accepted all four review annotations: restore exact helper-invocation validation, overall blocked precedence for incomplete required coverage, and repair of omitted stable prerequisites; retain the generation skill and exercise the three cases. Implement R28 in the maintenance body and existing documentation, with three reusable requests and artifact-based reviewer criteria. [SOURCE: docs/prd-v0.1.md#additional-confirmation] [SOURCE: docs/execution/specs/2026-09-08-shoshin-design.md#verification-maintenance-coverage-and-recovery-contract]

- [x] Restore the documented helper interface: command, arguments, working directory, runtime prerequisites, interpreter or executable permissions, and actual re-execution. An interpreter workaround alone does not prove a documented direct invocation.
- [x] Define blocked, clean and changed as overall coverage outcomes. Required blocked paths take precedence over partial local corrections; report those corrections separately. Explicit non-applicable or out-of-scope paths do not silently narrow coverage.
- [x] Correct missing stable prerequisites in the relevant recipe using source and execution evidence. Temporary service failures belong in run evidence; retain uncertainty when the distinction is unresolved.
- [x] Run three independent native fixture contexts against the candidate source. A repairs a non-executable helper, verifies list/export and discovers an unmapped sync-status command. B corrects the list response shape and map omissions while configured remote export is unavailable. C observes a stable export entitlement denial separately from a refused sync-service connection. All preserve product files and report blocked for required successful paths not verified, despite completed documentation or helper corrections.
- [x] Preserve a corrected premise: Python argparse accepts --data as a unique abbreviation of --dataset. Native and parent execution show that this spelling is not an observed failure. The actual original direct-invocation defect is missing executable permission. Do not count interface clarification as a second reproduced failure.
- [x] Add a fourth reachable control with only doctor/list/export in the actual CLI parser. Its native execution corrects the helper and documentation, verifies both mapped features and reports changed. It correctly excludes an unreachable source branch from the user feature map. Preserve the earlier A/B source-to-map discoveries and blockers rather than replacing their results.
- [x] Repeat the reachable control: fresh source inspection and exact documented execution report clean. Parent comparison confirms all nine fixture files retain their hashes and permissions. Independently replay 21 documented commands across all four projects, verify 16 unchanged product files and reread 73 evidence artifacts. Preserve four native contexts, five completed turns, initial snapshots, exact dispatches and results in tests/evidence/verification-maintenance-restoration.json.

These are bounded standard-library CLI projects, not full real-project or UI acceptance. Initial dispatches contain no inherited parent history or expected verdicts; the repeat uses the same context after its first completed run. No custom runtime, product repair, configuration change, service startup, installation, scheduling or remote publication is part of this revision. The create-verification-skill body, descriptions and invocation metadata remain unchanged. Existing real-project acceptance items stay open.

Validation: aggregate checks passed, including 46 document-governance tests, 18 Shoshin helper tests, Ruff and skill structure checks. Strict document validation passed with zero warnings. Evidence integrity checks passed for four contexts, five turns, five dispatches, 37 initial files, 23 changed or new verification-skill files, 73 artifacts and the candidate skill hash. The parent command extractor initially supported inline code only and stopped at a fenced shell block; after adding fenced-command parsing, the full replay passed without fixture changes. Whitespace checks passed.
