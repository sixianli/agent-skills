...
初始推进顺序为 P0 → P1 → P2 → P3 → P4。性能、取证等附属流程在基础能力形成后撰写与验证，P4 才接通入口。顺序不要求多代理；有独立且适合并行的任务时才委派。
...
- [ ] 删除固定 Cursor Task 参数、固定模型名和简单问题也必须委派的规定。
...
- [ ] 明确审查意图与范围，按需要提供独立审查输入，由主审核实、去重和分类。
...
- [ ] 默认直接复盘，确有独立分析需要才委派；核实提案来源和目标技能。
