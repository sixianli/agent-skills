...
`tests/evidence/shoshin-entrypoint-boundaries.json` adds scoped missing-authoring-capability handling, an actual refused connection to the designated project browser backend, and a fresh ordinary desktop task that implicitly reads shoshin and Feature, implements persisted pause/resume, and answers a status follow-up without tools. It also records 42 supplied-catalog decisions across full, 120-character and 60-character descriptions. Full host metadata remained visible in those offline contexts, so those decisions do not establish behavior under genuine host-generated truncation. The fresh task uses temporary code and does not complete the canceled real-project pilot or all package acceptance.
