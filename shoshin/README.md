...
This package is maintained in English only. Historical user statements and narrative evidence are English translations, not verbatim English transcripts. Original records remain in Git history. Timestamps, hashes, outcomes, and verification limits are preserved. Exact external paths containing non-English characters use JSON Unicode escapes, preserving the decoded locator without renaming external files. Test fixtures use English text while retaining Unicode coverage through an accented English word and a check-mark symbol.
